...
class ModelTemplate(metaclass=ABCMeta):
    def __init__(self, token_emb_mat, glove_emb_mat, tds, cds, tl, scope):
        ...
        self.sent1_char = tf.placeholder(tf.int32, [None, None, None], name='sent1_char')

        self.sent2_token = tf.placeholder(tf.int32, [None, None], name='sent2_token')
        self.sent2_char = tf.placeholder(tf.int32, [None, None, None], name='sent2_char')
    ...
    def build_loss(self):
        ...
        losses = tf.nn.sigmoid_cross_entropy_with_logits(
            labels=tf.reshape(self.gold_label, [-1,1]),
            logits=self.logits
        )
